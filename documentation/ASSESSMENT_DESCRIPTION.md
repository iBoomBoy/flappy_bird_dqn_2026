...
Please do not change the `config.yml` and any other non-submission files, e.g., `clock.py`. We won't use your version of those files in Gradescope, this is more for you to not experience any unintended game behaviours on your own PC.

## Assessment criteria
The assessment criteria are defined below:
* Lv 1 (5 marks) - your agent achieves an average score of 10 (here game_length = 10).
* Lv 2 (5 marks) - your agent achieves an average score >= 5 (here game_length = 10).
* Lv 3 (5 marks) - your agent's highest score is >= 5 (here game_length = 10).
* Lv 4 (5 marks) - your agent's highest score is >= 5 (here game_length = 10).
* Lv 5 (10 marks) - this one is a challenger level, your mark is calculated as:
`mark = min(max(highest_score - 4, 0), 10)`. This means you need to achieve a minimum score of 6 to get one mark, and achieves a minimum of 15 to get the full 10 marks. During the evaluation we will set game_length = 50 for this level. 

**After each level is completed the marking script should run the next level without rerunning the passed level(s).
However, if you made an error in a subsequent submission which led to the crash of the Gradescope autograder, the highest level
won't be remembered anymore, in such instances, you may resubmit your last working model to get through the passed levels first.**

## Submission 
You must submit your program files on the Gradescope portal linked in myLearning assignment page.
...
However, we will inspect your submission should Gradescope flag your submission with high code similarity to other students' submissions.
...
## Due date
This assessment's due date is specified on myLearning.
